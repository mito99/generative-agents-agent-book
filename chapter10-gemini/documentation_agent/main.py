...
class PersonaGenerator:
    def __init__(self, llm: BaseChatModel, k: int = 5):
        self.llm = llm
        self.parser = PydanticOutputParser(pydantic_object=Personas)
        self.k = k
    ...
    def run(self, user_request: str) -> Personas:
        ...
        return chain.invoke(
            {"user_request": user_request, "format_instructions": format_instructions}
        )
...
class InterviewConductor:
    ...
    def _generate_questions(
        ...
        # 質問をバッチ処理で生成
        return question_chain.batch(
            question_queries, max_concurrency=1, requests_per_second=1
        )
    ...
    def _generate_answers(
        ...
        # 回答をバッチ処理で生成
        return answer_chain.batch(
            answer_queries, max_concurrency=1, requests_per_second=1
        )
...
def main():
    ...
    parser.add_argument(
        ...
        default="スマートフォン向けの健康管理アプリを開発したい",
    )
    # "k"引数を追加
    parser.add_argument(
        "--k",
