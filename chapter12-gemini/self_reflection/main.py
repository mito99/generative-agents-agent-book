...
class TaskExecutor:
    ...
    def run(self, task: str, previous_results: str) -> str:
        ...
        result = agent.invoke(
            {
                "messages": [
                    (
                        ...
                        "4. 発見事項を明確に要約すること。\n"
                        "5. 前回タスク結果と重複する内容は回答しないこと。\n"
                        "6. 前回タスク結果があればそれを参考に回答してください。\n"
                        f"7. 以下の過去のふりかえりを考慮すること:\n{reflection_text}\n",
                    )
                ]
            }
        )
...
class ReflectiveAgent:
    ...
    def _execute_task(self, state: ReflectiveAgentState) -> dict[str, Any]:
        ...
        result = self.task_executor.run(
            task=current_task, previous_results=previous_results
        )
...
def main():
    ...
    parser.add_argument(
        "--task", type=str, help="実行するタスク", default="面白いゲームの作り方"
    )
    ...
    reflection_manager = ReflectionManager(
        file_path=settings.default_reflection_db_path
    )
