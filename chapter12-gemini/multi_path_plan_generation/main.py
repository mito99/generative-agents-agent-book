...
class QueryDecomposer:
    def __init__(self, llm: BaseChatModel):
        ...
        self.parser = PydanticOutputParser(pydantic_object=DecomposedTasks)

    def run(self, query: str) -> DecomposedTasks:
        prompt = ChatPromptTemplate.from_template(
            f"CURRENT_DATE: {self.current_date}\n"
            "-----\n"
            "タスク: 与えられた目標を必ず3〜5個の高レベルタスクに分解し、"
            "各タスクに必ず2〜3個の具体的なオプションを提供してください。\n"
            "要件:\n"
            "1. 以下の行動だけで目標を達成すること。決して指定された以外の行動をとらないこと。\n"
            "   - インターネットを利用して、目標を達成するための調査を行う。\n"
            "2. 各高レベルタスクは具体的かつ詳細に記載されており、単独で実行ならびに検証可能な情報を含めること。一切抽象的な表現を含まないこと。\n"
            "3. 各項レベルタスクに必ず2〜3個の異なるアプローチまたはオプションを提供すること。\n"
        ...
        formatted_instructions = self.parser.get_format_instructions()
        chain = prompt | self.llm | self.parser
        return chain.invoke(
            {"query": query, "format_instructions": formatted_instructions}
        )
...
class TaskExecutor:
    ...
    def run(self, task: Task, chosen_option: TaskOption, previous_results: str) -> str:
        ...
        result = agent.invoke(
            {
                "messages": [
                    (
                        ...
                        f"4. 発見事項を明確にまとめること。\n"
                        f"5. 回答は前回タスク結果があればそれを参考にしてください。\n"
                        f"6. 前回タスク結果と重複する内容は回答しないこと。\n",
                    )
                ]
            }
        )
...
class MultiPathPlanGeneration:
    ...
    def _execute_task(self, state: MultiPathPlanGenerationState) -> dict[str, Any]:
        ...
        previous_results = state.results[-1] if state.results else ""
...
def main():
    ...
    parser.add_argument(
        "--task", type=str, help="実行するタスク", default="カレーライスの作り方"
    )
