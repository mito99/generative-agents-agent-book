...
class RoleAssigner:
    def __init__(self, llm: BaseChatModel):
        self.llm = llm
        self.parser = PydanticOutputParser(pydantic_object=TasksWithRoles)
...
class Executor:
    ...
    def run(self, task: Task, previous_task_result: str) -> str:
                ...
                "messages": [
                    (
                        ...
                        (
                            ...
                            "前タスク結果があれば、前タスクを理解して回答してください。\n"
                            "回答は前タスク結果の内容を含めず回答してください。\n"
                            f"前タスク結果: {previous_task_result}\n"
                        ),
                    ),
                    (
                        "human",
        ...
        prompt = ChatPromptTemplate(
            [
                (
                    "system",
                    (
                        "あなたは総合的なレポート作成の専門家です。複数の情報源からの結果を統合し、"
                        "洞察力に富んだ包括的なレポートを作成する能力があります。"
                    ),
                ),
                (
                    "human",
...
class RoleBasedCooperation:
    ...
    def _execute_task(self, state: AgentState) -> dict[str, Any]:
        ...
        previous_task_result = (
            state.results[state.current_task_index - 1]
            if state.current_task_index > 0
            else ""
        )
        result = self.executor.run(
            task=current_task, previous_task_result=previous_task_result
        )
        return {
            "results": [result],
