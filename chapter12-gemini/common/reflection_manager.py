...
class Reflection(BaseModel):
    id: str = Field(
        ...
        description="リフレクション内容に一意性を与えるためのID",
    )
    task: str = Field(default="", description="ユーザーから与えられたタスクの内容")
    reflection: str = Field(
        default="",
        description=(
            "このタスクに取り組んだ際のあなたの思考プロセスを振り返ってください。"
            "何か改善できる点はありましたか? 次に同様のタスクに取り組む際に、"
            "より良い結果を出すための教訓を2〜3文程度で簡潔に述べてください。"
        ),
    )
    judgment: ReflectionJudgment = Field(
        default_factory=ReflectionJudgment, description="リトライが必要かどうかの判定"
    )
...
class TaskReflector:
    def __init__(self, llm: BaseChatModel, reflection_manager: ReflectionManager):
        self.llm = llm
        self.parser = PydanticOutputParser(pydantic_object=Reflection)
    ...
    def run(self, task: str, result: str) -> Reflection:
        prompt = ChatPromptTemplate.from_template(
            ...
            "リフレクション:\n"
            "このタスクに取り組んだ際のあなたの思考プロセスや方法を振り返ってくださ��。何か改善できる点はありましたか?\n"
